...
class TestVariantClient(unittest.TestCase):
    ...
    def test_caching(self):
        ...
        try:
            from_cache, pre_cache_r = cache_request(_getvariant)
            ...
            self.mv.set_caching(cache_name)

            # populate cache
            from_cache, cache_fill_r = cache_request(_getvariant)
            self.assertTrue(os.path.exists(cache_file))
            self.assertFalse(from_cache)
            # is it from the cache?
            from_cache, cached_r = cache_request(_getvariant)
            self.assertTrue(from_cache)

            self.mv.stop_caching()
            # same query should be live - not cached
            from_cache, post_cache_r = cache_request(_getvariant)
            self.assertFalse(from_cache)

            self.mv.set_caching(cache_name)
            # same query should still be sourced from cache
            from_cache, recached_r = cache_request(_getvariant)
            self.assertTrue(from_cache)

            self.mv.clear_cache()
            # cache was cleared, same query should be live
            from_cache, clear_cached_r = cache_request(_getvariant)
            self.assertFalse(from_cache)
            ...
            from_cache, first_getvariants_r = cache_request(_getvariants)
            del first_getvariants_r
            self.assertFalse(from_cache)
            # should be from cache this time
            from_cache, second_getvariants_r = cache_request(_getvariants)
